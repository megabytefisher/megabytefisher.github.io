...
class BubbleRow {
    ...
    constructor(images: HTMLImageElement[], wobbleArea: number, wobbleSpeed: number) {
        this.images = images;
...
class FishRenderer {
    ...
    async start() {
        this.fishImage = await loadImageAsync(imagePaths.fish);
        this.bubbleImages = [];
        for (let i = 0; i < imagePaths.bubbles.length; i++)
        {
            this.bubbleImages.push(await loadImageAsync(imagePaths.bubbles[i]));
        }
    ...
    drawImageRow(images: HTMLImageElement[], xOffset: number, yBase: number, xSpacing: number, rowHeight: number) {
...
const canvas = document.getElementById('kf-background') as HTMLCanvasElement;
